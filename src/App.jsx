...
function getModulations(kc, seed) {
  ...
  if (kc.modeIdx===5) {
    ...
    res.push({ label:"Relative Major", key:rm, mode:"Ionian", pivot:`${rm}`, pivotAbbrev:"", desc:`→ ${rm} Major`, color:"#4ECDC4" });
  } else {
    const rmin = NOTES[(ki+9)%12];
    res.push({ label:"Relative Minor", key:rmin, mode:"Aeolian", pivot:rmin, pivotAbbrev:"m", desc:`→ ${rmin} Minor`, color:"#A78BFA" });
  }
  const par = kc.modeIdx<=1 ? { key:kc.key, mode:"Aeolian", abbrev:"m", desc:`→ ${kc.key} Minor`, color:"#F59E0B" } : { key:kc.key, mode:"Ionian", abbrev:"", desc:`→ ${kc.key} Major`, color:"#F59E0B" };
  ...
  res.push({ label:"Random Pivot", key:rk, mode:"Ionian", pivot:dom, pivotAbbrev:"7", desc:`→ ${rk} via ${dom}7`, color:"#FB923C", isRandom:true });
  return res;
}
...
const QC = { major:"#4ECDC4", minor:"#A78BFA", dom:"#F59E0B", dim:"#F87171", aug:"#FB923C", sus:"#34D399", power:"#94A3B8", custom:"#9CA3AF" };
const qc = (q) => QC[q] || "#4ECDC4";
...
export default function SongwriterWorkbench() {
  ...
  const cc = workingChord ? qc(workingChord.type.quality) : "#4ECDC4";

  // ── RENDER ────────────────────────────────────────────────
  return (
    <div style={{fontFamily:"'Space Grotesk',system-ui,sans-serif",background:"#0B0804",minHeight:"100vh",color:"#EDE4D0",userSelect:"none",fontSize:13}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .label{font-family:'DM Mono',monospace;font-size:9px;letter-spacing:0.1em;color:#3A2510;text-transform:uppercase;}
      `}</style>

      {/* TOAST */}
      {toast&&<div style={{position:"fixed",bottom:20,left:"50%",transform:"translateX(-50%)",background:"#C9A84C",color:"#0B0804",padding:"7px 18px",borderRadius:20,fontSize:12,fontWeight:700,zIndex:9999,animation:"fadeIn 0.2s ease",pointerEvents:"none"}}>{toast}</div>}
      ...
      {aiModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:9000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setAiModal(null)}>
          <div style={{background:"#151008",border:"1px solid #2A1A08",borderRadius:12,padding:24,maxWidth:500,width:"100%",animation:"fadeIn 0.2s ease"}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
              <div style={{fontSize:13,fontWeight:700,color:"#C9A84C"}}>{aiModal.title}</div>
              <button className="btn" onClick={()=>setAiModal(null)} style={{background:"transparent",color:"#5A4020",fontSize:16,padding:"0 4px"}}>✕</button>
            </div>
            {aiModal.loading ? (
              <div style={{display:"flex",alignItems:"center",gap:10,color:"#5A4020",fontSize:12}}>
                <div style={{width:16,height:16,border:"2px solid #3A2510",borderTopColor:"#C9A84C",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>
                Asking Claude...
              </div>
            ) : (
              <div style={{fontSize:13,lineHeight:1.7,color:"#C0B090",whiteSpace:"pre-wrap"}}>{aiModal.text}</div>
            )}
          </div>
        </div>
      )}
      ...
      {/* HEADER */}
      <div style={{background:"#090604",borderBottom:"1px solid #1A1005",padding:"9px 16px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginRight:6}}>
          <div style={{width:30,height:30,background:"linear-gradient(135deg,#C9A84C,#7A5A10)",borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15}}>🎸</div>
          <div>
        ...
        <input value={projectName} onChange={e=>setProjectName(e.target.value)} style={{background:"transparent",border:"none",borderBottom:"1px solid #2A1A08",color:"#C9A84C",fontSize:12,fontWeight:600,padding:"1px 3px",width:140,outline:"none"}} placeholder="Project name..."/>

        {/* Genre */}
        <div style={{display:"flex",alignItems:"center",gap:4,marginLeft:4}}>
          <span className="label" style={{marginRight:2}}>Genre:</span>
          <select value={genre} onChange={e=>{const v=e.target.value;setGenre(v);setShowGenreInput(v==="Custom");}} style={{background:"#151008",border:"1px solid #2A1A08",color:"#C9A84C",padding:"4px 6px",borderRadius:5,fontSize:11,fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
            {Object.keys(GENRES).map(g=><option key={g} value={g}>{g}</option>)}
          </select>
          {showGenreInput&&<input value={customGenre} onChange={e=>setCustomGenre(e.target.value)} placeholder="e.g. Shoegaze..." style={{background:"#151008",border:"1px solid #2A1A08",color:"#EDE4D0",padding:"4px 8px",borderRadius:5,fontSize:11,width:120,outline:"none"}}/>}
        </div>

        <div style={{marginLeft:"auto",display:"flex",gap:5,flexWrap:"wrap",alignItems:"center"}}>
          <select value={customTuning?"custom":tuningKey} onChange={e=>{if(e.target.value!=="custom"){setTuningKey(e.target.value);setCustomTuning(null);setSelectedFrets({});}}} style={{background:"#151008",border:"1px solid #2A1A08",color:"#8A6830",padding:"4px 6px",borderRadius:5,fontSize:10,fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
            {Object.keys(TUNINGS).map(t=><option key={t} value={t}>{t}</option>)}
            {customTuning&&<option value="custom">Custom</option>}
          </select>
          <button className="btn" onClick={()=>setShowCustom(!showCustom)} style={{background:"#151008",border:"1px solid #2A1A08",color:"#8A6830",padding:"4px 8px",borderRadius:5}}>Custom ▾</button>
          <div style={{width:1,height:16,background:"#1A1005"}}/>
          <button className="btn" onClick={saveProject} style={{background:"#C9A84C",color:"#0B0804",padding:"4px 10px",borderRadius:5,fontWeight:700}}>Save</button>
          <button className="btn" onClick={()=>setShowProjects(!showProjects)} style={{background:"#151008",border:"1px solid #2A1A08",color:"#8A6830",padding:"4px 8px",borderRadius:5}}>Projects</button>
          <button className="btn" onClick={exportProject} style={{background:"#151008",border:"1px solid #2A1A08",color:"#8A6830",padding:"4px 8px",borderRadius:5}}>↓ Export</button>
          <label className="btn" style={{background:"#151008",border:"1px solid #2A1A08",color:"#8A6830",padding:"4px 8px",borderRadius:5,cursor:"pointer"}}>↑ Import<input type="file" accept=".json" style={{display:"none"}} onChange={importProject}/></label>
        </div>
      </div>

      {/* CUSTOM TUNING */}
      {showCustom&&(
        <div style={{background:"#0D0905",borderBottom:"1px solid #1A1005",padding:"7px 16px",display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
          <span className="label" style={{marginRight:4}}>Strings (6→1):</span>
          {customInputs.map((n,i)=><input key={i} value={n} maxLength={2} onChange={e=>{const v=[...customInputs];v[i]=e.target.value.toUpperCase();setCustomInputs(v);}} style={{width:32,background:"#0B0804",border:"1px solid #2A1A08",color:"#EDE4D0",padding:"3px",borderRadius:4,fontSize:11,textAlign:"center",fontFamily:"'DM Mono',monospace",outline:"none"}}/>)}
          <button className="btn" onClick={()=>{setCustomTuning([...customInputs]);setShowCustom(false);setSelectedFrets({});}} style={{background:"#C9A84C",color:"#0B0804",padding:"3px 10px",borderRadius:5,fontWeight:700}}>Apply</button>
        </div>
      )}

      {/* PROJECTS PANEL */}
      {showProjects&&(
        <div style={{background:"#0D0905",borderBottom:"1px solid #1A1005",padding:"12px 16px"}}>
          <div className="label" style={{marginBottom:8}}>Saved Projects</div>
          {Object.keys(projects).length===0?<div style={{fontSize:11,color:"#2A1A08"}}>No saved projects.</div>:(
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {Object.keys(projects).map(name=>(
                <div key={name} style={{background:"#151008",border:"1px solid #2A1A08",borderRadius:7,padding:"7px 10px",display:"flex",gap:8,alignItems:"center"}}>
                  <div>
                  ...
                  <button className="btn" onClick={()=>loadProject(name)} style={{background:"#C9A84C",color:"#0B0804",padding:"2px 8px",borderRadius:4,fontWeight:700}}>Load</button>
                  <button className="btn" onClick={()=>deleteProject(name)} style={{background:"transparent",border:"1px solid #2A1A08",color:"#5A3020",padding:"2px 6px",borderRadius:4}}>✕</button>
                </div>
              ))}
            </div>
          )}
      ...
      {/* INPUT MODE BAR */}
      <div style={{background:"#090604",borderBottom:"1px solid #1A1005",padding:"7px 16px",display:"flex",gap:6,alignItems:"center"}}>
        <button className="btn" onClick={()=>setInputMode("fretboard")} style={{background:inputMode==="fretboard"?"#2A1A08":"transparent",border:`1px solid ${inputMode==="fretboard"?"#3A2510":"#1A1005"}`,color:inputMode==="fretboard"?"#C9A84C":"#3A2510",padding:"3px 10px",borderRadius:5,fontWeight:600}}>Fretboard</button>
        <button className="btn" onClick={()=>setInputMode("text")} style={{background:inputMode==="text"?"#2A1A08":"transparent",border:`1px solid ${inputMode==="text"?"#3A2510":"#1A1005"}`,color:inputMode==="text"?"#C9A84C":"#3A2510",padding:"3px 10px",borderRadius:5,fontWeight:600}}>Type chord</button>
        {inputMode==="text"&&<>
          <input value={textInput} onChange={e=>handleTextInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&textChord)addToProgression(textChord);}} placeholder="Am7, Gsus4, C#maj7…" autoFocus style={{background:"#151008",border:"1px solid #2A1A08",color:"#EDE4D0",padding:"4px 10px",borderRadius:5,fontSize:12,outline:"none",width:200,fontFamily:"'DM Mono',monospace"}}/>
          {textChord&&<div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:15,fontWeight:700,fontFamily:"'DM Mono',monospace",color:qc(textChord.type.quality)}}>{textChord.root}{textChord.type.abbrev}</span><span style={{fontSize:10,color:"#5A4020"}}>{textChord.type.name}</span></div>}
          {textInput&&!textChord&&<span style={{fontSize:11,color:"#F87171"}}>Unrecognised</span>}
        </>}
      </div>

      {/* FRETBOARD */}
      {inputMode==="fretboard"&&(
        <div style={{overflowX:"auto",padding:"14px 16px 6px",background:"#090604",borderBottom:"1px solid #1A1005"}}>
          <div style={{minWidth:660}}>
            <div style={{display:"flex",marginLeft:40,marginBottom:2}}>
              {Array.from({length:FRETS+1},(_,f)=><div key={f} style={{width:f===0?36:50,flexShrink:0,textAlign:"center",fontSize:8,color:INLAY.has(f)?"#C9A84C":"#221508",fontFamily:"'DM Mono',monospace",fontWeight:INLAY.has(f)?600:400}}>{f===0?"nut":f}</div>)}
            </div>
            {tuning.map((on,si)=>si).reverse().map((si)=>{
              const on = tuning[si];
              return (
              <div key={si} style={{display:"flex",alignItems:"center",marginBottom:1}}>
                <div style={{width:40,textAlign:"right",paddingRight:7,fontSize:10,fontFamily:"'DM Mono',monospace",color:"#C9A84C",fontWeight:500,flexShrink:0}}>{on}</div>
                {Array.from({length:FRETS+1},(_,fr)=>{
                  ...
                  return (
                    <div key={fr} className="fc" onClick={()=>setSelectedFrets(p=>({...p,[k]:!p[k]}))} style={{width:fr===0?36:50,height:30,flexShrink:0,position:"relative",display:"flex",alignItems:"center",justifyContent:"center"}}>
                      {fr===0?<div style={{position:"absolute",right:0,top:0,bottom:0,width:4,background:"linear-gradient(180deg,#BBA880,#EDE4D0,#BBA880)",zIndex:1}}/>:<div style={{position:"absolute",left:0,top:0,bottom:0,width:2,background:"linear-gradient(180deg,#6A5028,#C9A84C,#6A5028)",borderRadius:1,zIndex:1}}/>}
                      <div style={{position:"absolute",left:0,right:0,height:thick,background:"linear-gradient(90deg,#3A2010,#6A4525,#3A2010)",zIndex:0}}/>
      ...
      {/* NOTES BAR */}
      <div style={{padding:"7px 16px",display:"flex",alignItems:"center",gap:5,borderBottom:"1px solid #1A1005",minHeight:38,flexWrap:"wrap"}}>
        <span className="label" style={{marginRight:4}}>Notes</span>
        {[...new Set(workingNotes)].map(n=><div key={n} style={{background:"#151008",border:`1px solid ${cc}44`,color:cc,padding:"2px 8px",borderRadius:20,fontSize:11,fontFamily:"'DM Mono',monospace",fontWeight:500}}>{n}</div>)}
        {workingNotes.length===0&&<span style={{fontSize:11,color:"#1E1208",fontStyle:"italic"}}>{inputMode==="fretboard"?"Tap frets to build a chord":"Type a chord name"}</span>}
        <div style={{marginLeft:"auto",display:"flex",gap:5}}>
          {inputMode==="fretboard"&&<button className="btn" onClick={()=>setSelectedFrets({})} style={{background:"transparent",border:"1px solid #1A1005",color:"#3A2510",padding:"2px 8px",borderRadius:4}}>Clear</button>}
          <button className="btn" onClick={()=>addToProgression(workingChord)} disabled={!workingChord} style={{background:workingChord?"#C9A84C":"#151008",color:workingChord?"#0B0804":"#1E1208",padding:"2px 12px",borderRadius:4,fontWeight:700,transition:"all 0.15s"}}>Add →</button>
        </div>
      </div>

      {/* PROJECT KEY BANNER */}
      {projectAnalysis && (
        <div style={{padding:"10px 16px",background:"#0D0A05",borderBottom:"1px solid #1A1005",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span className="label">Project key</span>
            <span style={{fontSize:18,fontWeight:700,fontFamily:"'DM Mono',monospace",color:"#4ECDC4"}}>{projectAnalysis.kc.key} {projectAnalysis.kc.mode}</span>
            {projectAnalysis.confident ? (
              <span style={{fontSize:9,color:"#34D399",fontFamily:"'DM Mono',monospace",background:"#0C2018",padding:"2px 6px",borderRadius:3}}>🔒 LOCKED · {projectAnalysis.fitPct}% fit</span>
            ) : (
              <span style={{fontSize:9,color:"#C9A84C",fontFamily:"'DM Mono',monospace",background:"#1A1308",padding:"2px 6px",borderRadius:3}}>ESTIMATING · add more chords</span>
            )}
          </div>
          {pinnedKC && (
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <span style={{fontSize:10,color:"#A78BFA",fontFamily:"'DM Mono',monospace"}}>📌 overridden → {pinnedKC.key} {pinnedKC.mode}</span>
              <button className="btn" onClick={()=>setPinnedKC(null)} style={{background:"#1A1005",border:"1px solid #2A1A08",color:"#8A6030",padding:"2px 8px",borderRadius:4,fontSize:9}}>Reset to detected</button>
            </div>
          )}
          {/* Out-of-key flags */}
          {(() => {
            ...
            return (
              <div style={{display:"flex",alignItems:"center",gap:6,marginLeft:"auto"}}>
                <span style={{fontSize:9,color:"#F59E0B",fontFamily:"'DM Mono',monospace"}}>⚠ Out of key:</span>
                {outliers.map(o=>{
                  const ch = progression[o.i].chord;
                  return <span key={o.i} style={{fontSize:11,fontFamily:"'DM Mono',monospace",color:"#F59E0B",background:"#1A1308",padding:"1px 6px",borderRadius:3}}>{ch.root}{ch.type.abbrev||""}</span>;
                })}
              </div>
            );
          })()}
      ...
      <div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:0,minHeight:280}}>
        ...
        {/* LEFT: Chord ID + Key */}
        <div style={{padding:"14px 16px",borderRight:"1px solid #1A1005"}}>
          ...
          {workingChord?(
            <div>
              <div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:2}}>
                ...
                <span style={{fontSize:18,fontWeight:500,color:"#C9A84C",fontFamily:"'DM Mono',monospace"}}>{workingChord.type.custom?(workingChord.type.abbrev==="*"?"?":workingChord.type.abbrev):(workingChord.type.abbrev||"maj")}</span>
              </div>
              <div style={{fontSize:11,color:"#5A4020",marginBottom:12}}>{workingChord.type.custom?"Unrecognised — saved as custom voicing":workingChord.type.name}</div>

              {workingChord.type.custom?(
                <div style={{color:"#5A4020",fontSize:11,lineHeight:1.6}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
                <div className="label">Key / Mode <span style={{color:"#1E1208"}}>— tap to pin</span></div>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:2,maxHeight:180,overflowY:"auto"}}>
                {keyContexts.map((kc,i)=>{
                  ...
                  return(
                    <div key={i} className="pill" onClick={()=>setPinnedKC(pinned?null:kc)} style={{display:"flex",alignItems:"center",gap:6,padding:"4px 6px",borderRadius:5,background:active?"#151008":"transparent",border:`1px solid ${active?"#2A1A08":"transparent"}`}}>
                      <span style={{fontSize:12,fontFamily:"'DM Mono',monospace",color:"#EDE4D0",fontWeight:600,minWidth:20}}>{kc.key}</span>
                      <span style={{fontSize:8,color:"#4ECDC4",fontFamily:"'DM Mono',monospace",background:"#0C2018",padding:"1px 4px",borderRadius:3}}>{kc.degree}</span>
                      <span style={{fontSize:10,color:"#4A3018"}}>{kc.mode}</span>
                      {pinned&&<span style={{fontSize:9,color:"#C9A84C",marginLeft:"auto"}}>📌</span>}
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{color:"#1E1208",fontSize:12,paddingTop:4}}>
        ...
        <div style={{display:"flex",flexDirection:"column"}}>
          {/* Tab bar */}
          <div style={{display:"flex",borderBottom:"1px solid #1A1005",padding:"0 16px"}}>
            {[["next","Next Chords"],["diatonic","Diatonic"],["modulate","Key Change"],["style","Style DNA"]].map(([id,label])=>(
              <button key={id} className="btn" onClick={()=>setActiveTab(id)} style={{padding:"9px 14px",borderRadius:0,background:"transparent",color:activeTab===id?"#C9A84C":"#3A2510",fontWeight:activeTab===id?700:400,borderBottom:activeTab===id?"2px solid #C9A84C":"2px solid transparent",fontSize:11}}>
                {label}
              </button>
            ))}
            {/* Genre AI button */}
            <button className="btn" onClick={handleAiGenre} style={{marginLeft:"auto",background:"#151008",border:"1px solid #2A1A08",color:"#8A6030",padding:"5px 10px",borderRadius:5,fontSize:10,alignSelf:"center",display:"flex",alignItems:"center",gap:5}}>
              ✨ {genre} ideas
            </button>
          </div>
                ...
                {nextChords.length>0?(
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                    {nextChords.map((s,i)=>{
                      ...
                      return(
                        <div key={i} style={{background:"#0F0A05",border:`1px solid ${col}30`,borderRadius:8,padding:"10px 8px",display:"flex",flexDirection:"column",gap:4}}>
                          ...
                          <div style={{fontSize:8,color:"#4ECDC4",fontFamily:"'DM Mono',monospace"}}>{s.degree}</div>
                          <button className="btn" onClick={()=>handleExplain(s,activeKC,`(${s.degree})`)} style={{background:"#1A1005",border:"1px solid #2A1A08",color:"#5A4020",padding:"2px 0",borderRadius:3,marginTop:2,fontSize:9}}>✨ Explain</button>
                        </div>
                      );
                    })}
                  </div>
                ):<div style={{fontSize:12,color:"#1E1208"}}>Add a chord first</div>}
              </div>
            )}
                ...
                {biasedDiatonic.length>0?(
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                    {biasedDiatonic.map((dc,i)=>{
                      ...
                      return(
                        <div key={i} style={{background:"#0F0A05",border:`1px solid ${col}${bias>1?"55":"22"}`,borderRadius:8,padding:"9px 8px",display:"flex",flexDirection:"column",gap:3}}>
                          ...
                          <div style={{fontSize:8,color:"#4ECDC4",fontFamily:"'DM Mono',monospace"}}>{dc.degree}</div>
                          {bias>1&&<div style={{fontSize:7,color:"#C9A84C",fontFamily:"'DM Mono',monospace"}}>★ {genre}</div>}
                          <button className="btn" onClick={()=>handleExplain(dc,activeKC,`(${dc.degree})`)} style={{background:"#1A1005",border:"1px solid #2A1A08",color:"#5A4020",padding:"2px 0",borderRadius:3,marginTop:2,fontSize:9}}>✨ Explain</button>
                        </div>
                      );
                    })}
                  </div>
            ...
            {activeTab==="modulate"&&(
              <div>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                  ...
                  <button className="btn" onClick={()=>setRandSeed(s=>s+1)} style={{background:"#151008",border:"1px solid #2A1A08",color:"#6A5030",padding:"2px 8px",borderRadius:4,fontSize:9}}>↻ New random</button>
                </div>
                {modulations.length>0?(
                  <div style={{display:"flex",flexDirection:"column",gap:7}}>
                    {modulations.map((m,i)=>(
                      <div key={i} style={{background:"#0F0A05",border:`1px solid ${m.color}22`,borderRadius:8,padding:"10px 12px",display:"flex",alignItems:"center",gap:12}}>
                        <div style={{flex:1}}>
                          ...
                          <div style={{fontSize:15,fontWeight:700,color:"#EDE4D0",fontFamily:"'DM Mono',monospace"}}>{m.desc}</div>
                        </div>
                        ...
                        <button className="btn" onClick={()=>handleExplain({root:m.pivot,type:CHORD_TYPES.find(c=>c.abbrev===m.pivotAbbrev)||CHORD_TYPES[0]},activeKC,`pivot to ${m.desc}`)} style={{background:"#151008",border:"1px solid #2A1A08",color:"#5A4020",padding:"4px 8px",borderRadius:4,fontSize:9}}>✨ Explain</button>
                      </div>
                    ))}
                  </div>
                ):<div style={{fontSize:12,color:"#1E1208"}}>Add a chord first</div>}
              ...
              <div>
                ...
                {styleProfile&&progression.length>=2?(
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                    <div style={{background:"#0F0A05",border:"1px solid #1A1005",borderRadius:8,padding:"10px 12px"}}>
                      <div className="label" style={{marginBottom:6}}>Tonality</div>
                      <div style={{fontSize:14,fontWeight:700,color:"#4ECDC4",fontFamily:"'DM Mono',monospace"}}>{styleProfile.tonality}</div>
                    </div>
                    <div style={{background:"#0F0A05",border:"1px solid #1A1005",borderRadius:8,padding:"10px 12px"}}>
                      <div className="label" style={{marginBottom:6}}>Complexity</div>
                      <div style={{fontSize:14,fontWeight:700,color:"#C9A84C",fontFamily:"'DM Mono',monospace"}}>{styleProfile.complexity}</div>
                    </div>
                    <div style={{background:"#0F0A05",border:"1px solid #1A1005",borderRadius:8,padding:"10px 12px"}}>
                      <div className="label" style={{marginBottom:6}}>Mood fingerprint</div>
                      <div style={{fontSize:14,fontWeight:700,color:"#A78BFA",fontFamily:"'DM Mono',monospace"}}>{styleProfile.mood}</div>
                    </div>
                    <div style={{background:"#0F0A05",border:"1px solid #1A1005",borderRadius:8,padding:"10px 12px"}}>
                    ...
                    {styleProfile.topPatterns.length>0&&(
                      <div style={{background:"#0F0A05",border:"1px solid #1A1005",borderRadius:8,padding:"10px 12px",gridColumn:"1/-1"}}>
                        <div className="label" style={{marginBottom:6}}>Repeated patterns</div>
                        {styleProfile.topPatterns.map(([p,c])=>(
                          <div key={p} style={{fontSize:10,fontFamily:"'DM Mono',monospace",color:"#5A4020",marginBottom:3}}>{p} <span style={{color:"#C9A84C"}}>×{c}</span></div>
                        ))}
                      </div>
                    )}
                  </div>
                ):(
                  <div style={{fontSize:12,color:"#1E1208"}}>Add at least 2 chords to build your style profile.</div>
                )}
              </div>
            )}
          </div>
      ...
      {/* PROGRESSION STRIP */}
      <div style={{borderTop:"1px solid #1A1005",padding:"12px 16px",background:"#090604"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
          <div className="label">Progression {progression.length>0&&`(${progression.length} chords)`}</div>
          {progression.length>0&&<button className="btn" onClick={()=>setProgression([])} style={{background:"transparent",border:"none",color:"#2A1A08",fontFamily:"'DM Mono',monospace",fontSize:9}}>CLEAR ALL</button>}
        </div>
        {progression.length===0?(
          <div style={{fontSize:11,color:"#1E1208",fontStyle:"italic"}}>Your progression appears here. Click any chord suggestion or "Add →" to build it.</div>
        ):(
            ...
            {progression.map((p,i)=>{
              ...
              return(
                <div key={i} style={{display:"flex",alignItems:"center",gap:3}}>
                  {i>0&&<div style={{color:"#1A1005",fontSize:12}}>→</div>}
                  <div style={{position:"relative"}}>
                    <div className="pill" onClick={()=>handleExplain(p.chord,p.keyContext,`(position ${i+1})`)} style={{background:"#0F0A05",border:`1px solid ${col}44`,borderRadius:7,padding:"6px 10px",textAlign:"center",minWidth:46}}>
                      ...
                      {p.keyContext&&<div style={{fontSize:7,color:"#2A1A08",fontFamily:"'DM Mono',monospace",marginTop:1}}>{p.keyContext.degree}/{p.keyContext.key}</div>}
                    </div>
                    <button className="btn" onClick={()=>setProgression(prev=>prev.filter((_,j)=>j!==i))} style={{position:"absolute",top:-5,right:-5,width:14,height:14,borderRadius:"50%",background:"#2A1008",border:"none",color:"#F87171",fontSize:8,display:"flex",alignItems:"center",justifyContent:"center",padding:0,lineHeight:1}}>✕</button>
                  </div>
                </div>
              );
            })}
